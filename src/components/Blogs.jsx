...
function Blogs() {
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged(function(user){
            if(user){
                if(user.uid==="1l1U9VryPGc0HY4oShwObqOgLSF2"){
                    console.log("U are the Admin")
                    setadmin(true)
                }
            }
            else{
                    console.log("U are not the admin")
                    setadmin(false)
            }
          })

        axios.get("https://blog-project-backend-1gjd.onrender.com/api/blogs").then((res) => {
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`https://blog-project-backend-1gjd.onrender.com/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post("https://blog-project-backend-1gjd.onrender.com/api/blogs", { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
